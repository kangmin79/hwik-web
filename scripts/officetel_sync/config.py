...
MOLIT_NUM_OF_ROWS = 9999               # 2026-04-27: 아파트(collect_trades_v2.py)와 동일. 1페이지 종료 보장.
MOLIT_TIMEOUT_SEC = 60                 # 2026-04-27: 15→60. 아파트와 동일.
MOLIT_MAX_RETRY = 3
MOLIT_PAGE_CAP = 3000                  # v6 파이프라인 기준 (2026-04-17 헬리오시티 잘림 사고 반영)

# 운영키 100만/일, 30 QPS 한도. 워커 6 + 17 process burst 모델 (아파트와 동일).
# 토큰 버킷은 사실상 비활성 — 워커 수로 자연 분산.
MOLIT_GLOBAL_QPS = 100
...
# ── 병렬 ──────────────────────────────────────────────────
TRADE_WORKERS = 6                      # 2026-04-27: 28→6 (아파트와 동일, 토큰 경쟁 제거)
BLDG_WORKERS = 28
KAKAO_WORKERS = 16
UPLOAD_WORKERS = 8

# ── 회로 차단기 ────────────────────────────────────────────
CIRCUIT_BREAKER_CONSEC_FAIL = 30       # 2026-04-27: 10→30 (운영키 진짜 차단 외 미발동)
