...
function makeDongSlug(gu,dong,addr){var a=(addr||'').split(/\s+/),r=a[0]?(_RM[a[0]]||''):'',p=[];if(r){p.push(r);if(!_MC.has(r)&&r!=='세종'){if(a[1])p.push(a[1].replace(/(시|군)$/,''));if(a[2]&&a[2].endsWith('구'))p.push(a[2]);}else{if(gu.endsWith('군'))p.push(gu.replace(/군$/,''));else p.push(gu);}}else{p.push(gu);}(dong||'').split(' ').forEach(function(d){p.push(d);});var s=p.join('-');return s.replace(/[^\w가-힣]/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'');}
