...
### 수정 작업 원칙 (⚠️ 강제)
**대량 페이지(danji/dong/gu/ranking 등)나 공통 컴포넌트 수정 시 반드시:**
1. **1개 페이지에 먼저 적용** (ONE_DANJI_ID 또는 단일 타겟)
2. **사용자 확인** — 시각·구조·JSON-LD 모두 OK 받기
3. **확인 후 전체 적용**
4. **배포**

- 단계 건너뛰고 전체 적용 금지 (2026-04-19 클로킹 7회 수정 누적 롤백 사고 방지)
- "조금만 고치면 되니까 바로 전체" 유혹 금지 — 예외 없음
- ONE_DANJI_ID 같은 미리보기 환경변수 없으면 먼저 추가부터
